...
describe("pure command preflight", () => {
      ...
      for (const permissionProfile of ["strict", "balanced", "trusted"] as const) {
        for (const target of ["opencode", "claude", "codex"] as const) {
          for (const probe of ["pwd", "ls -d .", "command -v ms-agent-kit"]) {
            expect(commandPreflight(operation(probe), target, role, { ...context, permissionProfile })).toMatchObject({ decision: target === "opencode" ? "allow" : "unknown", effects: { status: "known", writes: [] }, runtime: "unknown" })
          ...
          for (const command of ["pwd -P", "ls", "ls -d /repo", "ls -d ..", "ls .env", "ls -d . extra", "command -v node", "command -v ms-agent-kit extra", "command ms-agent-kit", "ls -d . && command -v ms-agent-kit", "pwd; pwd", "ls -d $(pwd)", "pwd > out", "sh -c pwd", "bash -c pwd", "env pwd", "ms-agent-kit project inspect", "npm test", "npm run build"]) {
            expect(commandPreflight(operation(command), target, role, { ...context, permissionProfile }).decision, `${role}: ${command}`).toBe("deny")
          }
        }
      }
    }
  ...
  it("preserves the static API while separating permissions from uninspected script effects", () => {
    expect(staticCommandDecision("npm run test", "ms-tester", context)).toBe("allow")
    ...
    expect(result).toMatchObject({ decision: "allow", policy: { decision: "allow" }, effects: { status: "unknown", writes: null }, services: { status: "unknown", required: null }, runtime: "unknown" })
    expect(result.reasons.join(" ")).toContain("cachés")
  })

  it("distinguishes allow, ask, deny and unknown without borrowing another client's rules", () => {
    expect(commandPreflight(operation("pwd"), "opencode", "ms-codex", context).decision).toBe("allow")
    expect(commandPreflight(operation("other-command"), "opencode", "ms-codex", context).decision).toBe("ask")
    ...
    for (const target of ["opencode", "claude", "codex"] as Target[]) {
      for (const command of ["cat .env", "cat ../secrets/token", "rm -rf src", "git reset --hard", "npm test && cat .env"]) {
        expect(commandPreflight(operation(command), target, "ms-codex", context).decision, `${target}: ${command}`).toBe("deny")
      }
      for (const command of ["make test", "docker compose run test", "./verify.sh"]) {
        ...
        if (target === "opencode" && command === "make test") expect(result.decision).toBe("allow")
        else expect(result.decision).not.toBe("allow")
  ...
  it("separa consultas conocidas de scripts y no usa efectos desconocidos como denegación", () => {
    ...
    for (const [role, command] of [["ms-fastlane", "pnpm build"], ["ms-fastlane", "rg TODO src"], ["ms-tester", "node node_modules/vitest/vitest.mjs run"]] as const) {
      expect(commandPreflight(operation(command), "opencode", role, context)).toMatchObject({ decision: "allow", policy: { decision: "allow" }, effects: { status: "unknown", writes: null }, runtime: "unknown" })
    }
    expect(commandPreflight(operation("docker compose run test"), "opencode", "ms-tester", context).decision).toBe("deny")
  })
  ...
  it("retains provenance and never grants workspace paths from cwd metadata", () => {
    ...
    for (const target of ["opencode", "claude", "codex"] as const) {
      expect(commandPreflight(operation("git diff -- src"), target, "ms-designer", context).decision).toBe("deny")
    }
  })
  ...
  it("muestra concesiones personales exactas y salidas autorizadas sin inferir efectos", async () => {
    ...
    for (const role of ["ms-codex", "ms-fastlane", "ms-tester"] as const) {
      for (const command of commands) {
        expect(staticCommandDecision(command, role, granted)).toBe("allow")
        expect(commandPreflight(operation(command), "opencode", role, granted)).toMatchObject({ decision: "allow", policy: { decision: "allow", source: "/home/.ms-agent-kit/config.yaml#verification.projects" }, effects: { status: "unknown", writes: null }, runtime: "unknown", projectAuthorization: { command: true, outputPaths: ["coverage", "test-results"] } })
      }
    }
    ...
    for (const candidate of [commandPreflight(operation(commands[0]!, "nested"), "opencode", "ms-tester", granted), commandPreflight(operation(commands[0]!), "opencode", "ms-tester", { ...granted, scope: "user" }), commandPreflight(operation(commands[0]!), "opencode", "ms-tester", { ...granted, projectRoot: path.join(root, "other") }), commandPreflight(operation(commands[0]!), "opencode", "ms-designer", granted)]) {
      expect(candidate.decision).not.toBe("allow")
      expect(candidate.projectAuthorization).toEqual({ command: false, outputPaths: [], source: null })
    }
    expect(commandPreflight(operation(`${commands[0]} extra`), "opencode", "ms-tester", granted).decision).toBe("deny")
    expect(commandPreflight(operation(`${commands[0]} && cat .env`), "opencode", "ms-tester", granted).decision).toBe("deny")
  })
})
