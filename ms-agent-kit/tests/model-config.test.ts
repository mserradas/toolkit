...
describe("configuración de modelos y preferencias por cliente", () => {
  ...
  it("conserva preferencias documentales de proyecto como instrucciones", async () => {
    const context = await fixture()
    ...
    expect(artifact(artifacts, "codex", "agent", "ms-writer").content.toString()).not.toContain('"documentation/guides/**" = "write"')
  ...
  it("habilita skills técnicas conservando prohibiciones y distingue fastlane de status", async () => {
    ...
    for (const name of ["ms-codex", "ms-fastlane", "ms-tester"]) {
      ...
      expect(claude.disallowedTools).toBeUndefined()
      expect(artifact(artifacts, "codex", "agent", name).content.toString()).toContain("Puedes cargar skills técnicas seleccionadas")
    }
    expect(parseMarkdown(artifact(artifacts, "claude", "command", "ms-fastlane").content.toString()).frontmatter).toMatchObject({ context: "fork", agent: "ms-fastlane" })
