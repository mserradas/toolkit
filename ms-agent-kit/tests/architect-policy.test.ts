...
// External native rules are fixture data, not a policy generated by the kit.
const EXTERNAL_BASH_RULES = { "cat .env": "deny" }
const EXTERNAL_READ_RULES = { ".env": "deny" }
...
describe("cross-client delegation and acceptance policy", () => {
  it("keeps preflight, evidence ownership and denial handling in common sources", async () => {
    ...
    expect(sources[1]).toContain("los agentes no dependen de él")
...
describe.skipIf(jqMissing)("OpenCode doctor output budget (requires jq)", () => {
  it("uses native diagnostics without a global kit executable and leaves omissions unverified", async () => {
    const { source } = await openCodeDoctorFilters()
    expect(source).not.toContain("ms-agent-kit doctor")
    expect(source).toContain("opencode debug agent ms-architect")
  ...
  it("summarizes seven native agents without prompts or tool schemas", async () => {
    ...
    const results = ["ms-architect", "ms-codex", "ms-fastlane", "ms-tester", "ms-debugger", "ms-plan", "ms-discovery"].map((name) => {
      const role = { edit: "deny", skill: "allow" }
      const bash = { "*": "allow", "git push *": "ask" }
      const rules = { ...role, bash: { ...bash, ...EXTERNAL_BASH_RULES }, read: EXTERNAL_READ_RULES }
      ...
      expect(result.value.permissions.read.deny).toBe(Object.keys(EXTERNAL_READ_RULES).length)
      return result
    })
    expect(results.reduce((sum, result) => sum + result.bytes, 0) + Number(filters[1]![1])).toBeLessThan(16384)
    const changed = projectDoctorFixture(filters[0]!, { permission: [
  ...
  it("counts all skills, pages only metadata, and replaces oversized summaries", async () => {
    ...
    const result = projectDoctorFixture(filters[1]!, skills, 4)
    ...
    for (const [index, fixture] of [
      [0, { name: "🙂".repeat(10_000) }],
      [1, Array.from({ length: 4 }, () => ({ name: "🙂".repeat(100), description: "🙂".repeat(200), location: "🙂".repeat(300) }))],
    ] as const) {
...
describe("ms-architect policy", () => {
  ...
  it("assigns plan cadence, bounded retries, and one verification owner", async () => {
    ...
    for (const policy of [
      ...
      "ciérralo al aceptar",
      "continúa mientras haya progreso",
      "No cortes una misión por alcanzar un número prefijado",
  ...
  it("routes operational work and documents audited timeout fallbacks", async () => {
    ...
    expect(docs).toContain("la ejecución confía en el código del proyecto")
  ...
  it("documents permissive development and bounded documentary inspection", async () => {
    ...
    const docs = await readFile(path.join(DEFAULT_ASSETS_ROOT, "docs", "agents.md"), "utf8")
    expect(docs).toContain("El kit no añade políticas de permisos en ningún cliente")
    expect(docs).toContain("La opción antigua `--permission-profile` se ha retirado")
    ...
    expect(debuggerPrompt).toContain("permisos efectivos del cliente")
    expect(debuggerPrompt).not.toContain("edit: deny")
    expect(docs).not.toContain("declara `task: deny`")
  ...
  it("keeps role responsibilities separate from native permissions and user questions primary-only", async () => {
    const docs = await readFile(path.join(DEFAULT_ASSETS_ROOT, "docs", "agents.md"), "utf8")

    expect(docs).toContain("Las herramientas disponibles dependen del cliente")
    expect(docs).toContain("OpenCode recibe `permission: {}`")
  ...
  it("keeps fastlane direct and risk-based while preserving worker validation", async () => {
    ...
    expect(command).not.toContain("hooks de validación")
  ...
  it("builds project context from native reads without requiring persistence", async () => {
    const init = await readFile(path.join(DEFAULT_ASSETS_ROOT, "skills", "ms-project-init", "SKILL.md"), "utf8")
    expect(init).toContain("herramientas nativas")
    expect(init).toContain("la ausencia de `.agents/project.yaml` no bloquea el trabajo")
    expect(init).not.toMatch(/ms-agent-kit (?:project|doctor)/)
  ...
  it("keeps worker loops focal and reports reusable evidence", async () => {
    const agentPolicies = {
      ...
      "ms-fastlane": [
        "No mantienes planes ni TODOs",
        "No detengas la misión por un contador de ciclos",
        "agrupa lecturas independientes",
