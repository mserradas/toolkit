...
import { AGENT_DEFINITIONS, agentDefinition, agentToolCycleBudget } from "../src/core/agent-catalog.js"
...
describe("configuración observable de modelos y presupuestos", () => {
  it("aplica 32 solo a ms-codex en OpenCode y conserva todos los demás presupuestos", async () => {
    const { artifacts } = await fixture()
    for (const role of Object.keys(AGENT_DEFINITIONS)) {
      for (const target of TARGETS) expect(agentToolCycleBudget(role, target)).toBe(role === "ms-codex" && target === "opencode" ? 32 : agentDefinition(role).toolCycleBudget)
    }
    const content = (target: Target) => artifacts.find((item) => item.target === target && item.name === "ms-codex" && item.kind === "agent")!.content.toString()
    expect(parseMarkdown(content("opencode")).frontmatter).toMatchObject({ steps: 32, variant: "high" })
    expect(parseMarkdown(content("claude")).frontmatter.maxTurns).toBe(20)
    expect(content("codex")).toContain("Presupuesto operativo objetivo: 20 ciclos de herramienta.")
  ...
  it("separa hashes coincidentes de ajustes efectivos desconocidos", async () => {
    ...
    expect(rows.find((row) => row.target === "opencode" && row.role === "ms-codex")).toMatchObject({
      declared: { model: "openai/gpt-5.6-sol", reasoningEffort: "high", modelSource: "src/core/agent-catalog.ts#ms-codex", budget: { value: 32, mechanism: "steps" } },
      installed: { status: "comprobado", model: "openai/gpt-5.6-sol", reasoningEffort: "high", source: expect.stringContaining("ms-codex.md") },
    })
    for (const row of rows) expect(row.effective).toMatchObject({ status: "no comprobado", model: null, reasoningEffort: null, budget: null, source: null })
  ...
  it("preserva overrides con fuente sin aplicarlos al arquitecto Codex como skill", async () => {
    ...
    expect(rows.find((row) => row.role === "ms-codex")).toMatchObject({
      ...
      installed: { model: "custom-model", reasoningEffort: "low", budget: { value: 20, mechanism: "instruction" } },
    })
    const architect = rows.find((row) => row.role === "ms-architect")!
    expect(architect).toMatchObject({ materialization: "main-task", declared: { appliesToAgent: false, model: "architect-declared", reasoningEffort: "medium" }, installed: { status: "comprobado", model: null, reasoningEffort: null, budget: null } })
  ...
  it.each(["modified", "missing", "unmanaged", "different", "update", "absent"])("no infiere valores instalados para %s", async (state) => {
    ...
    expect(row.installed).toMatchObject({ status: "no comprobado", model: null, reasoningEffort: null, budget: null })
  ...
  it("conserva desconocidos cuando no hay plan válido", () => {
    ...
    expect(rows.find((row) => row.role === "ms-codex")?.declared).toMatchObject({ model: null, reasoningEffort: null, modelSource: "tarea principal del cliente", budget: { value: 20, mechanism: "maxTurns" } })
  })
